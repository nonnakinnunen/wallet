...
public class HandlerForWin implements RequestHandler<HandlerForWin.WinRequest, HandlerForWin.WinResponse> {
  ...
  @Override
  /*
   * Takes in an WinRequest and saves it to DynamoDB table
   */
  public WinResponse handleRequest(HandlerForWin.WinRequest event, Context context) {
    ...
    try{
      ...
      if(account != null){
        ...
        Integer currentBalance = account.getInt(ATTRIBUTE_CURRENT_BALANCE);
        ...
        transactionItem.put(ATTRIBUTE_TRANSACTION_TYPE, new AttributeValue().withS("WIN"));
        transactionItem.put(ATTRIBUTE_TIMESTAMP, new AttributeValue().withS(LocalDateTime.now().toString()));
        transactionItem.put(ATTRIBUTE_AMOUNT, new AttributeValue().withN(event.winAmount().toString()));
        ...
        Integer newBalance = currentBalance + event.winAmount();
        ...
        expressionAttributeValues.put(":newBalance", new AttributeValue().withN(newBalance.toString()));
        ...
        client.transactWriteItems(placeWinTransaction);
          return new WinResponse(newBalance);
          ...
          return new WinResponse(currentBalance);
          }
          throw e;
      }
...
public static record WinRequest(Integer accountId, Integer transactionId, Integer winAmount) {
}
public static record WinResponse(Integer balance) {
}
}
